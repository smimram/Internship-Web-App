...
public class TopicManagementServlet extends HttpServlet {
    ...
    private List<Topic> getTopics(String orderByColumn, String orderBySort) {
        ...
        try {
            ...
            String query = "SELECT DISTINCT id, title, institution, program_id, administr_validated, scientific_validated, confidential_internship, timestamp_fiche, timestamp_report, timestamp_slides, creation_date "
                    + "FROM internship "
            ...
            try (
                ...
                while (resultSet.next()) {
                    ...
                    topic.setDateUpload(resultSet.getTimestamp("creation_date"));
                    topics.add(topic);
                }
            }
            return topics;
    ...
    private HashMap<String, ArrayList<Category>> getAllCategories(List<Program> programs) {
        ...
        try {
            ...
            for (Program p : programs) {
                String query = "SELECT DISTINCT c.* FROM program_category pc, categories c WHERE pc.cat_id = c.id AND pc.program_id = ? ORDER BY description";
                PreparedStatement preparedStatement = con.prepareStatement(query);
