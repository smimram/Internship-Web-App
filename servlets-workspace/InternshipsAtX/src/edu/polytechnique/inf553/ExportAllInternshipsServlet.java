...
//                String query = "'\\copy ( SELECT i.id AS internshipId, i.title AS internshipTitle, i.institution AS intersnhipInstituion, i.creation_date AS internshipCreationDate, i.scientific_validated AS scientificValidation, i.administr_validated AS administrativeValidation, p.name AS supervisorName, p.email AS supervisorEmail, CONCAT(pr.name, pr.year) AS program, i.confidential_internship, p2.name AS studentName, p2.email AS studentEmail FROM internship i LEFT JOIN person p ON i.supervisor_id = p.id LEFT JOIN program pr ON i.program_id = pr.id LEFT JOIN person_internship pi ON pi.internship_id = i.id LEFT JOIN person p2 ON pi.person_id = p2.id) TO /tmp/exports/export.csv'";
//                System.out.println(query);
//                ProcessBuilder builderLoad = new ProcessBuilder("psql", "-d", "internship_webapp", "-c", query);
